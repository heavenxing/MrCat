...
Options specifically tailored for bet_macaque:
  ...
  -s <size>   method to estimate the brain size. Choose from: "int"(ensity) (default), "bet", or give the <back to front distance in mm>. For the last option 70mm is a good guess.
...
while [[ $c -lt $niter ]] ; do
  ...
  if [[ $flgTP == true ]] ; then
    ...
    [[ $(fslstats ${base}_TpoleL_mask -R | awk '($1==$2){print "TRUE"}') == TRUE ]] && fslmaths ${base}_TpoleL_mask -mul 0 ${base}_TpoleL_mask
    [[ $(fslstats ${base}_TpoleR_mask -R | awk '($1==$2){print "TRUE"}') == TRUE ]] && fslmaths ${base}_TpoleR_mask -mul 0 ${base}_TpoleR_mask
  fi
  ...
  if [[ $(fslstats ${base}_brain_mask -R | awk '($1==$2){print "TRUE"}') == TRUE ]] ; then
    >&2 echo "Error: I'm sorry, the brain mask is empty."
    >&2 echo "This most likely an issue with the initial estimate of the brain size."
    >&2 echo "Run the command with '-d' to debug to inspect the initial mask."
