...
#--------------------------------------------------------------------------
# Registration of functional/diffusion (EPI) to structural (T1) images.
#
# Usage
...
Register functional/diffusion (EPI) and structural (T1) data.

examples:
  register_func_struct.sh --epi=/path/to/funcdata/mean_epi
      --t1=/path/to/structdata/struct --t1brain=/path/to/structdata/struct_brain
      --segmentT1 --brainmask --biascorr --register
  register_func_struct.sh --epi=func --t1=struct --all

usage: register_func_struct.sh
  INPUT specification
...
# retrieve diffusion and structural directories
epidir=$(dirname $EPI)
...
# [[ -z $transdir ]] && transdir="$structdir/../transform" #### old code
transdir="$structdir/../transform" #### Rogier fix 2016-06-08
...
if [[ $instr =~ --segmentT1$ ]] ; then
  ...
  if [[ -r $T1wm.nii.gz ]] ; then
    ...
    tmpdir=$(mktemp -d "/tmp/segmentT1.XXXXXXXXXX")
    imcp $T1brain $tmpdir/${T1}_brain

    # segment the T1, without extra biascorrection
    fast -n 3 -t 1 -g --nobias $tmpdir/${T1}_brain
    immv $tmpdir/${T1}_brain_seg_2 $T1wm
...
if [[ $instr =~ --brainmask$ ]] ; then
  ...
  if [[ $flg_bet_epi == TRUE ]] ; then
    ...
    $MRCATDIR/in_vivo/bet_macaque.sh $epidir/$EPI $tmpdir/$EPI -t T2star #-f 0.5 -fFP 0.9 -fTP 0.9
    immv $tmpdir/${EPI}_brain_mask $EPIbrainmask

    # remove the tmpdir, and a potention strict mask (that is no longer valid)
    rm -rf $tmpdir
...
if [[ $instr =~ --biascorr$ ]] ; then
  ...
  if [[ ! -r ${EPIbrainmask}_strict.nii.gz ]] ; then
    ...
    fslmaths $EPIbrainmask -s 2 -thr $thr -binv -mas $EPIbrainmask ${EPIbrainmask}_strict
  fi

  # create a tmpdir
  tmpdir=$(mktemp -d "/tmp/EPI_biascorr.XXXXXXXXXX")
  ...
  $MRCATDIR/HCP_scripts/RobustBiasCorr.sh \
    --in=$epidir/$EPI \
    --workingdir=$tmpdir \
  ...
  imcp $tmpdir/${EPI}_restore $epidir/${EPI}_restore
  imcp $tmpdir/${EPI}_bias $epidir/${EPI}_bias
...
if [[ $instr =~ --register$ ]] ; then
  ...
  if [[ $flg_fmap == TRUE ]] && [[ ! -r $fmapmagbrain.nii.gz ]]; then
    ...
    if [[ $flg_fmapreg == TRUE ]] ; then
      ...
      # create a tmpdir
      tmpdir=$(mktemp -d "/tmp/fmapmagbrainmask.XXXXXXXXXX")
      # resample the T1 brain mask to the field map
      flirt -in $T1brainmask -ref $fmapmag -applyxfm -init $FSLDIR/etc/flirtsch/init.mat -out $tmpdir/brainmask
      fslmaths $fmapmag -mas $tmpdir/brainmask $fmapmagbrain
      # remove the tmpdir
      rm -rf $tmpdir
    fi
  fi
  ...
  regfwd=$transdir/${EPI}_to_${T1base}
  reginv=$transdir/${T1base}_to_${EPI}
